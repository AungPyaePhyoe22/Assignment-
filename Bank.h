...
// from-WinHtut-to-lonelone-100
    ...
    if(who == 't'){
        ...
        for(int i=0; i<25; i++){
            ...
            db[transfer].trc[space_array[transfer]-15].note[index_point]=Ctime[0].current_time[i];
            index_point++;
        }
        ...
        TIME();
        for(int a=0; a<25; a++){
            db[receiver].trc[space_array[receiver]-15].note[index_point]=Ctime[0].current_time[a];
            index_point++;
        }
